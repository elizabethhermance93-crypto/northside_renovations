...
if($("#contact-form").length){
    ...
    var $steps = $contactForm.find('.form-step');
    var $stepPoints = $contactForm.find('.step-point');
    var currentStep = 0;
    ...
    function goToStep(index){
        ...
        $stepPoints.removeClass('active completed');
        $stepPoints.each(function(i){
            if(i < currentStep){
    ...
    $contactForm.on('click', '.step-next', function(e){
        e.preventDefault();
    ...
    $contactForm.on('click', '.step-prev', function(e){
        e.preventDefault();
        goToStep(currentStep - 1);
    });
...
(function () {
  function hideElfsightCredit() {
    const links = document.querySelectorAll('.es-widget-background-content > a:last-of-type');
    links.forEach(el => {
  ...
  document.addEventListener('DOMContentLoaded', hideElfsightCredit);

  // Observe aggressive re-injection
  const observer = new MutationObserver(hideElfsightCredit);
